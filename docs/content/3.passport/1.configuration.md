...
| `strategies.<name>.endpoints.user`      | Authenticated user endpoint  <br/> Property containing authenticated user data | `{ url: '/api/user', method: 'get', property:'profile' }`                                                                                 |
| `strategies.<name>.endpoints.logout`    | Custom name used internally for the request                                    | `{ alias: 'Custom name' }`                                                                                                                |
| `strategies.<name>.endpoints.twoFactor` | Two-factor authentication endpoint                                             | `{ url: '/api/send-token-2fa', method: 'post', alias: 'Custom name', property: 'access_token', expires: 'expires_in',headerName: '2fa' }` |

For complete typings, check the source
file: [auth.d.ts](https://github.com/4sllan/nuxt-umbu/blob/main/src/templates/auth-types.ts).
...
1. The client posts credentials to `/oauth/token`.

2. The app receives and stores the access and refresh tokens.

3. The app uses the access token when requesting `/api/profile` (or the configured user endpoint).

4. The authenticated user data is stored in the application state.
