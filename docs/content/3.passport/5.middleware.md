...
<script setup>
definePageMeta({
  middleware: ['auth', 'twoFactor'] // Multiple middleware
})
</script>
```
...
export default defineEventHandler(async (event) => {
  ...
  return { data: 'Protected data', user: user.id }
})
```
...
```typescript [plugins/middleware.client.ts]
...
export default defineNuxtRouteMiddleware(async (to) => {
  ...
  const { loggedIn } = useAuth()
  const result = loggedIn.value ? undefined : navigateTo('/login')
...
import { createTestClient } from 'h3'
