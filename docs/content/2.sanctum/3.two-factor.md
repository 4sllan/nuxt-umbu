...
In **Nuxt Umbu**, Two-Factor Authentication is completely optional.

It is only enabled when the `twoFactor` endpoint is configured within the selected strategy.  
...
const handleVerify = async () => {
    try {
        await $auth.twoFactor('admin', code)
        // success
    } catch (err) {
        // error
    }
        ...
        strategies: {
            client: {
                ...
                endpoints: {
                    ...
                    twoFactor:  { url: '/api/token-2fa', method: 'post' }, // If present → 2FA enabled
...
### Then:
- `twoFactor()` method should not be used
- `umbu:two-factor` middleware should not be applied
