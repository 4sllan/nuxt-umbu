---
title: Configuration
description: How to configure Laravel Sanctum authentication in Nuxt Umbu
navigation:
  icon: i-lucide-cog
---

Nuxt Umbu integrates seamlessly with Laravel Sanctum to provide secure, cookie-based authentication for your Nuxt
application.

This guide explains how to configure the Sanctum provider.

---

## Initial setup

The only required configuration is the `baseURL`, which points to your Laravel API.

```typescript [nuxt.config.ts]
export default defineNuxtConfig({
    modules: ['nuxt-umbu'],

    auth: {
        provider: 'sanctum'
    },

    runtimeConfig: {
        public: {
            baseURL: 'http://localhost:80', // Your Laravel API
        }
    },
})
```

## Available options

Below are the available authentication options when using the sanctum provider.

| Parameter                               | Description                                                                   | Default                                                  |
|-----------------------------------------|-------------------------------------------------------------------------------|----------------------------------------------------------|
| `csrf`                                  | Endpoint used to retrieve CSRF cookie                                         | `/sanctum/csrf-cookie`                                   |
| `provider`                              | Authentication provider                                                       | `sanctum`                                                |
| `strategies.<name>.redirect.login`      | Redirect path after login                                                     | `undefined`                                              |
| `strategies.<name>.redirect.logout`     | Redirect path after logout                                                    | `undefined`                                              |
| `strategies.<name>.endpoints.login`     | Login endpoint configuration                                                  | `{ url: '/login', method: 'post' }`                      |
| `strategies.<name>.endpoints.user`      | Authenticated user endpoint  <br/>Property containing authenticated user data | `{ url: '/api/user', method: 'get', property: profile }` |
| `strategies.<name>.endpoints.logout`    | Logout endpoint                                                               | `{ url: '/logout', method: 'post' }`                     |
| `strategies.<name>.endpoints.twoFactor` | Two-factor authentication endpoint                                            | `undefined`                                              |

For complete typings, check the source file: [auth.d.ts](https://github.com/4sllan/nuxt-umbu/blob/main/src/templates/auth-types.ts).

## Full Configuration Example

Below is a complete example using Sanctum with custom endpoints and redirects.

```typescript [nuxt.config.ts]
export default defineNuxtConfig({
    modules: ['nuxt-umbu'],

    auth: {
        csrf: '/sanctum/csrf-cookie',
        provider: 'sanctum',
        strategies: {
            client: {
                redirect: {
                    logout: "/auth",
                    login: "/auth"
                },
                endpoints: {
                    login: {url: "/login", method: "post"},
                    user: {url: "/api/profile", method: "get", property: "profile"},
                    twoFactor: {url: "/api/token-2fa", method: "post"},
                    logout: {url: "/api/logout", method: "post"}
                },
            }
        }
    },

    runtimeConfig: {
        public: {
            baseURL: 'http://localhost:80',
        },
    },
})
```

## Understanding the Flow

When using Sanctum, the authentication flow works as follows:

1. The application requests `/sanctum/csrf-cookie`.

2. Laravel returns the `XSRF-TOKEN` cookie.

3. The client sends credentials to `/login`.

4. Laravel creates an authenticated session.

5. The app requests `/api/profile` (or the configured user endpoint).

6. The authenticated user data is stored in the application state.

All of this process is handled automatically by Nuxt Umbu.

## Overriding Configuration

You can customize behavior inside the `auth` object:

```typescript [nuxt.config.ts]
export default defineNuxtConfig({
    modules: ['nuxt-umbu'],

    auth: {
        strategies: {
            client: {
                redirect: {
                    login: '/dashboard',
                    logout: '/'
                }
            }
        }
    }
})
```

## Using Environment Variables

It is recommended to configure `baseURL using environment variables:

### .env

```env
NUXT_PUBLIC_BASE_URL=http://localhost:80
```

```typescript [nuxt.config.ts]
export default defineNuxtConfig({
    modules: ['nuxt-umbu'],

    runtimeConfig: {
        public: {
            baseURL: process.env.NUXT_PUBLIC_BASE_URL
        }
    }
})
```

